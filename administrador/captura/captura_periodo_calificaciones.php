				...
				<div class="container">
					<div class="row gtr-200">
						<?php
								...
								//echo "SELECT unidad.id as id1,unidad.unidad , periodocalificacion.id as id2, periodocalificacion.fecha_inicio, periodocalificacion.fecha_fin FROM unidad	INNER JOIN unidadperiodo ON	unidad.id = unidadperiodo.unidad INNER JOIN	periodocalificacion	ON unidadperiodo.periodoCalificacion = periodocalificacion.id WHERE	periodocalificacion.ciclo = '".$_SESSION["ciclo"]."'"; 
								$con1= mysqli_query($conexion,"SELECT unidad.id as id1,unidad.unidad , periodocalificacion.id as id2, periodocalificacion.fecha_inicio, periodocalificacion.fecha_fin FROM unidad	INNER JOIN unidadperiodo ON	unidad.id = unidadperiodo.unidad INNER JOIN	periodocalificacion	ON unidadperiodo.periodoCalificacion = periodocalificacion.id WHERE	periodocalificacion.ciclo = '".$_SESSION["ciclo"]."'") or die(mysqli_error($conexion)); 
								$_SESSION["id1"]=array();
								$_SESSION["unidad"]=array();
								$_SESSION["id2"]=array();
								$_SESSION["fecha_inicio"]=array();
								$_SESSION["fecha_fin"]=array();
								while ($row = mysqli_fetch_array($con1,MYSQLI_NUM)){ 
									$_SESSION["id1"][]=$row[0];
									$_SESSION["unidad"][]=$row[1];
									$_SESSION["id2"][]=$row[2];
									$_SESSION["fecha_inicio"][]=$row[3];
								...
								echo "<div class='col-3 col-12-mobile' id='sidebar'>
									...
									<section>
										<header>
											<h3>PERIODO DE CALIFICACIONES</h3>
										</header>
									</section>
								</div>";
								
								if(count($_SESSION["id1"]) != 0){//ACTUALIZAR
									echo "<div class='col-8 col-12-mobile imp-mobile' id='content'>
											<article id='main'>
												<form id='form1' name='form1' method='post' onSubmit = 'return validation(this)' action='almacenar_periodo_calificaciones.php' autocomplete='off'>";
										
										echo "<table width=500 align=center border='2'> 
											<tr bgcolor='bbbbbb' align=center> 
											<td style='border:1px solid white'><center>UNIDAD</center></td> 
										...
										for($i=0;$i<count($_SESSION["id1"]);$i++){ 
											echo "<tr "; 
											if ($num_fila%2==0) 
											...
											echo"
												...
												<input type='hidden' name='idp[$num_fila]' id='idp[$num_fila]' value='".$_SESSION["id2"][$i]."'>
												<td style='border:1px solid white'><center><input style='font-size:14px; font-family: arial' type='date' name='fechai[$num_fila]' id='fechai[$num_fila]' value='".$_SESSION["fecha_inicio"][$i]."' onkeypress='return soloLetras(event)' maxlength='5' size='5'  tabindex='$cont1' onkeydown='return simularTab(event, this, 13,37,38,39,40)' /></center></td>
												<td style='border:1px solid white'><center><input style='font-size:14px; font-family: arial' type='date' name='fechaf[$num_fila]' id='fechaf[$num_fila]' value='".$_SESSION["fecha_fin"][$i]."' onkeypress='return soloLetras(event)' maxlength='5' size='5'  tabindex='$cont1' onkeydown='return simularTab(event, this, 13,37,38,39,40)' /></center></td>";
										...
										echo "</table>
											...
											<input type='hidden' name='materias' id='materias' value='$nmateria'>
											</p>
											</form>
											</article>
											</div>";
