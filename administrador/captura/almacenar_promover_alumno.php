<?php	
		...
		for($i=0;$i<=count($matricula);$i++){
			if($cuadro[$i]=='on'){
				...
				if($grado[$i]<18){
					...
					//echo "INSERT INTO alumnofisica (educacionFisica, alumno, ciclo) values ('".$deporte[$i]."', '".$matricula[$i]."', '".$_SESSION["ciclo"]."')";
					...
					//echo "INSERT INTO alumnoingles (ingles, alumno, ciclo) values ('".$ingles."', '".$matricula[$i]."', '".$_SESSION["ciclo"]."')";
	...
	if($sql5_1 or $sql5_2) {
		echo "<script languaje='javascript'>alert('Se guardaron los datos correctamente');window.location='Inicio_promover_alumno.php';</script>";
		//echo "Se guardaron los datos correctamente";
	}
	else{
		echo "<script languaje='javascript'>alert('No se pudieron guardar los datos');window.location='Inicio_promover_alumno.php';</script>";
