					...
					<div class="row gtr-200">
						<?php
							...
							echo "<div class='col-3 col-12-mobile' id='sidebar'>
									...
									<section>
										<header>
											<h3>ASIGNATURA: ESPA&Ntilde;OL</h3>
										</header>
							...
							for($i=0;$i<count($alumno);$i++){
								$Con = mysqli_query($conexion,"select comentarios.bloqueo from comentarios where (comentarios.alumno = '".$alumno[$i]."') and (comentarios.unidad = '".$_SESSION["bloque"]."') and (comentarios.ciclo = '".$_SESSION["ciclo"]."') and (comentarios.tipo = '".utf8_encode('Español')."') and (comentarios.bloqueo = '1')") or die(mysqli_error($conexion));
								$row = mysqli_fetch_array($Con,MYSQLI_ASSOC);
								if($row['bloqueo'] == 1)
									$bloqueo[$i]=$row['bloqueo'];
							}
							for($i=0;$i<count($alumno);$i++){
								$Con = mysqli_query($conexion,"select comentarios.id from comentarios where (comentarios.alumno = '".$alumno[$i]."') and (comentarios.unidad = '".$_SESSION["bloque"]."') and (comentarios.ciclo = '".$_SESSION["ciclo"]."') and (comentarios.tipo = '".utf8_encode('Español')."')") or die(mysqli_error($conexion));
								$row = mysqli_fetch_array($Con,MYSQLI_ASSOC);
								if($row['id'] != 0 ){
									$datos[$i]=$row['id'];
								}
							...
							if(count($bloqueo) != 0){
								...
								if(count($datos) != 0){//ACTUALIZAR
									for($i=0;$i<count($alumno);$i++){
										$Con = mysqli_query($conexion,"select comentarios.id, comentarios.comentario from comentarios where comentarios.tipo = '".utf8_encode('Español')."' AND (comentarios.alumno = '".$alumno[$i]."') and (comentarios.unidad = '".$_SESSION["bloque"]."') and (comentarios.ciclo = '".$_SESSION["ciclo"]."')") or die(mysqli_error($conexion));
										$row = mysqli_fetch_array($Con,MYSQLI_ASSOC);
										$idMat[$i]=$row['id'];
									...
									echo "<div class='col-8 col-12-mobile imp-mobile' id='content'>
											<article id='main'>
												<form id='form1' name='form1' method='post' onSubmit = 'return validation(this)' action='almacenar_calificacion_comentarios_primaria.php' autocomplete='off'>
													<p>
													<center>";
