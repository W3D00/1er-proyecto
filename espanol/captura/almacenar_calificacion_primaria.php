<?php	
	...
	IF($boton == 'Capturar'){
		...
		for($i=0;$i<count($materia);$i++){
			for($j = 1; $j < $no_filas; $j++) {
				if($materia[$i]=='ESP001' || $materia[$i]=='ESP002' || $materia[$i]=='ESP003' || $materia[$i]=='ESP004' || $materia[$i]=='ESP005' || $materia[$i]=='ESP006'){
					$sql = mysqli_query($conexion,"INSERT INTO calificacionmateria (id, alumno, materia, unidad, ciclo, noClases, asistencia, inasistencia, evidencias, porcentajeEvidencias, knotion, porcentajeKnotion, suma, porcentajeFinal, bloquear) values (NULL, '".$ids[$j]."', '".$materia[$i]."', '".$_SESSION['bloque']."', '".$_SESSION["ciclo"]."', '".$clase."', '".$asist[$j]."', NULL, '".$calificacion[$i][$j]."', '".$porcentajeCalificacion[$i][$j]."', '".$knotion[$i][$j]."', '".$porcentajeKnotion[$i][$j]."', '".$suma[$i][$j]."', '".$final[$i][$j]."', '0')") or die ('Error: '.mysqli_error($conexion));
				}
				else if($materia[$i]=='ESE001' || $materia[$i]=='ESE002' || $materia[$i]=='ESE003' || $materia[$i]=='ESE004' || $materia[$i]=='ESE005' || $materia[$i]=='ESE006'){
					$sql = mysqli_query($conexion,"INSERT INTO calificacionmateria (id, alumno, materia, unidad, ciclo, noClases, asistencia, inasistencia, evidencias, porcentajeEvidencias, knotion, porcentajeKnotion, suma, porcentajeFinal, bloquear) values (NULL, '".$ids[$j]."', '".$materia[$i]."', '".$_SESSION['bloque']."', '".$_SESSION["ciclo"]."', NULL, NULL, NULL, '".$calificacion[$i][$j]."', '".$porcentajeCalificacion[$i][$j]."', NULL, NULL, '".$suma[$i][$j]."', '".$final[$i][$j]."', '0')") or die ('Error: '.mysqli_error($conexion));
				}
				else{
					$sql = mysqli_query($conexion,"INSERT INTO calificacionmateria (id, alumno, materia, unidad, ciclo, noClases, asistencia, inasistencia, evidencias, porcentajeEvidencias, knotion, porcentajeKnotion, suma, porcentajeFinal, bloquear) values (NULL, '".$ids[$j]."', '".$materia[$i]."', '".$_SESSION['bloque']."', '".$_SESSION["ciclo"]."', NULL, NULL, NULL, '".$calificacion[$i][$j]."', '".$porcentajeCalificacion[$i][$j]."', '".$knotion[$i][$j]."', '".$porcentajeKnotion[$i][$j]."', '".$suma[$i][$j]."', '".$final[$i][$j]."', '0')") or die ('Error: '.mysqli_error($conexion));
				}
			}
		}
	}
	if($sql == false) {
		echo "<script languaje='javascript'>alert('No se pudieron guardar los datos');window.location='Inicio_captura_calificaciones.php';</script>";
	}
	else{
		echo "<script languaje='javascript'>alert('Se guardaron los datos correctamente');window.location='Inicio_captura_calificaciones.php';</script>";
	}
