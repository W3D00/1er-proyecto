					...
					<div class="row gtr-200">
						<?php
							...
							if($_SESSION["grado"] == 7 || $_SESSION["grado"] == 8 || $_SESSION["grado"] == 10 || $_SESSION["grado"] == 11 || $_SESSION["grado"] == 13 || $_SESSION["grado"] == 14 || $_SESSION["grado"] == 16 || $_SESSION["grado"] == 17){
								//echo "estoy aqui";
								//HAcer la consulta
								$con1= mysqli_query($conexion,"SELECT DISTINCT educacionfisica.id FROM maestro, maestrodeporte, educacionfisica, gradoeducacionfisica, grado WHERE concat (maestro.nombre, ' ', maestro.paterno, ' ', maestro.materno) like '".$_SESSION['username']."' AND maestro.clave = maestrodeporte.maestro AND maestrodeporte.deportes = educacionfisica.id AND educacionfisica.id = gradoeducacionfisica.educacionFisica AND gradoeducacionfisica.grado = grado.id AND grado.id = '".$_SESSION["grado"]."'") or die(mysqli_error($conexion)); 
								while ($row = mysqli_fetch_array($con1,MYSQLI_NUM)){ 
									...
									if(count($datos) != 0){
										...
										while ($damefila=mysqli_fetch_object($result)){ 
											...
											if ($num_fila%2==0)
												...
												echo "<td style='border:1px solid white' bgcolor='F8E68B'>";
											echo "<center><input style='font-size:14px; font-family: arial' type='text' name='fisica[$num_fila]' id='fisica[$num_fila]' value='0' maxlength='5' size='5' ONCHANGE = 'calculate(this.id)' onkeypress='return'  onFocus='javascript:this.value=\"\"' tabindex='$cont1' onkeydown='return simularTab(event, this, 13,37,38,39,40)'/></center></td>
											<input type='hidden' name='pfisica[$num_fila]' id='pfisica[$num_fila]' value='0' readonly/>
											<input type='hidden' name='sumafisica[$num_fila]' id='sumafisica[$num_fila]' value='0' readonly/>
											<input type='hidden' name='ffisica[$num_fila]' id='ffisica[$num_fila]' value='0' readonly/>
											</tr> ";
											//aumentamos en uno el número de filas 
											$num_fila++;
										} //cierro el while 
							...
							if($_SESSION["grado"] == 19 || $_SESSION["grado"] == 20 || $_SESSION["grado"] == 22 || $_SESSION["grado"] == 23){
								...
								if($_SESSION["grado"] == 19 || $_SESSION["grado"] == 20){
									if($deporte=="BASQUETBOL"){$_SESSION["deporte"]=7;}
									if($deporte=="FUTBOL"){$_SESSION["deporte"]=9;}
									if($deporte=="VOLEIBOL"){$_SESSION["deporte"]=5;}
								} 
								if($_SESSION["grado"] == 22 || $_SESSION["grado"] == 23){
									if($deporte=="BASQUETBOL"){$_SESSION["deporte"]=8;}
									if($deporte=="FUTBOL"){$_SESSION["deporte"]=10;}
									if($deporte=="VOLEIBOL"){$_SESSION["deporte"]=6;}
								}
