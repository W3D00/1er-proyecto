					...
					<!--CENTER-->
						<?php
							...
							for($i=0;$i<count($alumno);$i++){
								$Con = mysqli_query($conexion,"select comentarios.bloqueo from comentarios where (comentarios.alumno = '".$alumno[$i]."') and (comentarios.unidad = '".$_SESSION["bloque"]."') and (comentarios.ciclo = '".$_SESSION["ciclo"]."') and (comentarios.tipo = '".utf8_encode('Inglés')."') and (comentarios.bloqueo = '1')") or die(mysqli_error($conexion));
							...
							for($i=0;$i<count($alumno);$i++){
								...
								$Con = mysqli_query($conexion,"select comentarios.id from comentarios where (comentarios.alumno = '".$alumno[$i]."') and (comentarios.unidad = '".$_SESSION["bloque"]."') and (comentarios.ciclo = '".$_SESSION["ciclo"]."') and (comentarios.tipo = '".utf8_encode('Inglés')."')") or die(mysqli_error($conexion));
								$row = mysqli_fetch_array($Con,MYSQLI_ASSOC);
								//valores de las consultas
								if($row['id'] != 0 ){
									$datos[$i]=$row['id'];
							...
							if(count($bloqueo) != 0){
								...
								if(count($datos) != 0){//Actualizar
									...
									echo "<div class='col-8 col-12-mobile imp-mobile' id='content'>
											<article id='main'>
												<form id='form1' name='form1' method='post' onSubmit = 'return validation(this)' action='almacenar_comentarios_ingles.php' autocomplete='off'>
													<p>
													<center>";
									for($i=0;$i<count($alumno);$i++){
										$Con = mysqli_query($conexion,"select comentarios.id, comentarios.comentario from comentarios where comentarios.tipo = '".utf8_encode('Inglés')."' AND (comentarios.alumno = '".$alumno[$i]."') and (comentarios.unidad = '".$_SESSION["bloque"]."') and (comentarios.ciclo = '".$_SESSION["ciclo"]."')") or die(mysqli_error($conexion));
										$row = mysqli_fetch_array($Con,MYSQLI_ASSOC);
										$idMat[$i]=$row['id'];
									...
									for($i=0;$i<count($alumno);$i++){
										...
										if ($num_fila%2==0) 
											...
											echo "bgcolor=#58D3F7";
										echo ">"; 
										echo"<td style='border:1px solid white'><font size='2'>$num_fila</font></td> 
											<td nowrap style='border:1px solid white'>".$nombre[$i]."</td>
											<input type='hidden' name='hexa[$num_fila]' id='hexa[$num_fila]' value='".$idMat[$i]."'>
									...
									echo "</table>
										...
										<center>
											<input type='submit' name='Capturar' id='Capturar' value='Actualizar' tabindex='800'/>
										</center>
