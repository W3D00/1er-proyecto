				...
				<div class="container"-->
					<div class="row gtr-200">
						<?php
							...
							$con1= mysqli_query($conexion,"SELECT club.id, club.club FROM club INNER JOIN maestroclub ON maestroclub.club = club.id INNER JOIN gradoclub ON gradoclub.club = club.id INNER JOIN grado ON gradoclub.grado = grado.id INNER JOIN maestro ON maestroclub.maestro = maestro.clave WHERE maestro.usuario = '".$_SESSION['usuario']."' AND grado.id = '".$_SESSION["grado"]."'") or die(mysqli_error($conexion)); 
							while ($row = mysqli_fetch_array($con1,MYSQLI_NUM)){ 
								$_SESSION["club"]=$row[0];
								$_SESSION["nombreclub"]=$row[1];
							}
							//-----------------------------//
								...
								if(count($datos) != 0){
									...
									while ($damefila=mysqli_fetch_object($result)){ 
										...
										if ($num_fila%2==0)
												...
												echo "<td style='border:1px solid white' bgcolor='F8E68B'>";
											echo "<center><input style='font-size:14px; font-family: arial' type='text' name='tec[$num_fila]' id='tec[$num_fila]' value='0' maxlength='5' size='5' ONCHANGE = 'calculate(this.id)' onkeypress='return'  onFocus='javascript:this.value=\"\"' tabindex='$cont1' onkeydown='return simularTab(event, this, 13,37,38,39,40)'/></center></td>
											<input type='hidden' name='ptec[$num_fila]' id='ptec[$num_fila]' value='0' readonly/>
											<input type='hidden' name='sumatec[$num_fila]' id='sumatec[$num_fila]' value='0' readonly/>
											<input type='hidden' name='ftec[$num_fila]' id='ftec[$num_fila]' value='0' readonly/>
											</tr> ";
										//aumentamos en uno el n\u00famero de filas 
