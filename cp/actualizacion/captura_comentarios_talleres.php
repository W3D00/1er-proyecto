					...
					<div class="row gtr-200">
						...
						<?php
							...
							for($i=0;$i<count($alumno);$i++){
								$Con = mysqli_query($conexion,"select comentarios.bloqueo from comentarios where (comentarios.alumno = '".$alumno[$i]."') and (comentarios.unidad = '".$_SESSION["bloque"]."') and (comentarios.ciclo = '".$_SESSION["ciclo"]."') and (comentarios.tipo = '".utf8_encode('Taller')."') and (comentarios.bloqueo = '1')") or die(mysqli_error($conexion));
								$row = mysqli_fetch_array($Con,MYSQLI_ASSOC);
								if($row['bloqueo'] == 1)
									$bloqueo[$i]=$row['bloqueo'];
							}
							for($i=0;$i<count($alumno);$i++){
								$Con = mysqli_query($conexion,"select comentarios.id from comentarios where (comentarios.alumno = '".$alumno[$i]."') and (comentarios.unidad = '".$_SESSION["bloque"]."') and (comentarios.ciclo = '".$_SESSION["ciclo"]."') and (comentarios.tipo = '".utf8_encode('Taller')."')") or die(mysqli_error($conexion));
								$row = mysqli_fetch_array($Con,MYSQLI_ASSOC);
								if($row['id'] != 0 ){
									$datos[$i]=$row['id'];
								}
