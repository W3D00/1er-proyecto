					...
					<div class="row gtr-200">
						...
						<?php
							...
							for($i=0;$i<count($alumno);$i++){
								$Con = mysqli_query($conexion,"select comentarios.id, comentarios.comentario from comentarios where comentarios.tipo like '".mb_convert_encoding('Español', 'UTF-8', 'ISO-8859-1')."' AND (comentarios.alumno = '".$alumno[$i]."') and (comentarios.unidad = '".$_SESSION["bloque"]."') and (comentarios.ciclo = '".$_SESSION["ciclo"]."')") or die(mysqli_error($conexion));
								$row = mysqli_fetch_array($Con,MYSQLI_ASSOC);
								if( isset($row['comentario']) )
									$comentarios[$i]=$row['comentario']." ";
							}
							for($i=0;$i<count($alumno);$i++){
								$Con = mysqli_query($conexion,"select comentarios.id, comentarios.comentario from comentarios where comentarios.tipo = '".mb_convert_encoding('Inglés', 'UTF-8', 'ISO-8859-1')."' AND (comentarios.alumno = '".$alumno[$i]."') and (comentarios.unidad = '".$_SESSION["bloque"]."') and (comentarios.ciclo = '".$_SESSION["ciclo"]."')") or die(mysqli_error($conexion));
								$row = mysqli_fetch_array($Con,MYSQLI_ASSOC);
								if( isset($row['comentario']) )
									$comentarios[$i]=$comentarios[$i].$row['comentario']." ";
							}
							for($i=0;$i<count($alumno);$i++){
								$Con = mysqli_query($conexion,"select comentarios.id, comentarios.comentario from comentarios where comentarios.tipo = '".mb_convert_encoding('Taller', 'UTF-8', 'ISO-8859-1')."' AND (comentarios.alumno = '".$alumno[$i]."') and (comentarios.unidad = '".$_SESSION["bloque"]."') and (comentarios.ciclo = '".$_SESSION["ciclo"]."')") or die(mysqli_error($conexion));
								$row = mysqli_fetch_array($Con,MYSQLI_ASSOC);
								if( isset($row['comentario']) )
									$comentarios[$i]=$comentarios[$i].$row['comentario'];
							}
							echo "<div class='col-8 col-12-mobile imp-mobile' id='content'>
									<article id='main'>
										<form id='form1' name='form1' method='post' onSubmit = 'return validation(this)' action='almacenar_comentario_taller.php' autocomplete='off'>
											<p>
											<center>";
							echo "<table width=500 align=center border='2'> 
								<tr bgcolor='bbbbbb' align=center> 
								<td style='border:1px solid white; width:15px'><center>NO</center></td> 
								<td style='border:1px solid white; width:50px'><center>NOMBRE</center></td>
							...
							for($i=0;$i<count($alumno);$i++){
								...
								echo"
									...
									<td><center>".$comentarios[$i]."</center></td>
									</tr>";
