				...
				<div class="container">
					<div class="row gtr-200">
						...
						<div class="col-7 col-12-mobile imp-mobile" id="content">
							<article id="main">
								<form id="form1" name="form1" method="post" action="captura_calificacion_primaria_talleres.php" onSubmit = "return validar(this)" enctype='multipart/form-data'>
									<?php
										...
										if(isset($calificacion)){
											echo "<center>
												<table>
												<tr>";
											$consulta_mysql="SELECT grado.id, concat(grado.grado,'', grupo.descripcion) AS gg FROM nivel, nivelgrado, grado, grupo, maestroartes, gradoarte, maestro WHERE (nivel.id = '3') AND (nivel.id = nivelgrado.nivel) AND (nivelgrado.grado = grado.id) AND (grado.grupo = grupo.id) AND (grupo.descripcion <> 'U') AND grado.id = gradoarte.grado AND gradoarte.artes = maestroartes.artes AND maestroartes.maestro = maestro.clave AND maestro.usuario = '".$_SESSION['usuario']."' ORDER BY gg ASC";
											$resultado_consulta_mysql=mysqli_query($conexion,$consulta_mysql);
											mysqli_close($conexion);
											echo "<td><b>GRUPO</b></td><td><select name='Grado' id='Grado' onChange='activar1(this.form)' style='width: auto;'>";
